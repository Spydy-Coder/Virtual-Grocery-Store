...
function Card({ imageSrc, title, items }) {
    return (
      <div className="parent flex flex-row justify-center h-90 w-90 ml-10 mr-10 shadow-md rounded-xl bg-white pl-10 pr-10  pt-5 border border-red-500 mt-3">
        <div className="child1 ">
          <img src={imageSrc} alt="" className="img h-80 w-90" />
        </div>
        <div className="child2 h-90 w-101 ml-6 ">
          <ul className="list">
            <li>
              <h1 className="head text-2xl mb-8  text-green-500 ml-3 font-bold">{title}</h1>
            </li>
            {items.map((item, index) => (
              <li key={index} className="list2 flex gap-7 m-3 ">
                <img
  ...
  function Card1({ imageSrc, title, items }) {
    return (
      <div className="parent flex flex-row justify-center h-90 w-90 ml-10 mr-10 shadow-md rounded-xl bg-white pl-10 pr-10  pt-5 border border-red-500 mt-3 mb-6">
        ...
        <div className="child2 h-90 w-101 mr-6 ">
          <ul className="list">
            ...
            {items.map((item, index) => (
              <li key={index} className="list2 flex gap-7 mb-3 mt-3 mr-3 ">
                <img
                  src="https://cdn-icons-png.flaticon.com/128/2203/2203239.png"
                  alt=""
                  className="imgs w-8 h-8 "
